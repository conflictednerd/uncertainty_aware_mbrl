...
# from wm import SimpleWorldModel
...
def test_wm(cfg: DictConfig):
    ...
    if 'normalizer.running_mean' in checkpoint:
        checkpoint['normalizer.running_mean'] = checkpoint['normalizer.running_mean'].squeeze(0)

    if 'normalizer.running_var' in checkpoint:
        checkpoint['normalizer.running_var'] = checkpoint['normalizer.running_var'].squeeze(0)

    wm.nn.load_state_dict(checkpoint)
    ####test dataset with 400,000 ds  ID
    with open(f"runs/{'button-press-v2-goal-observable__test_exp__42__1732571954'}/ds.pkl", "rb") as f:
        ds = pickle.load(f)
    wm.test(ds)
